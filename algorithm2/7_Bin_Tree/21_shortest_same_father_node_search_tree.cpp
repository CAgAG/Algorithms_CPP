 ...
 * @encode : UTF-8
 * @note   : 二叉搜索树的最近公共祖先 https://leetcode.cn/problems/lowest-common-ancestor-of-a-binary-search-tree/description/
 * @date   : 2024/3/15 13
 *************************/
/* 一个节点也可以是它自己的祖先 */
/* 注意:
...
public:
    TreeNode *traversal(TreeNode *root, TreeNode *p, TreeNode *q) {
        ...
        if (root->val > p->val || root->val > q->val) {
            left_node = traversal(root->left, p, q);
        }
        if (root->val < p->val || root->val < q->val) {
            right_node = traversal(root->right, p, q);
        }
...
int main() {
    ...
    // 树中节点数目在范围 [2, 105] 内
    vector<int> tree_nums = {6, 2, 8, 0, 4, 7, 9, null_num, null_num, 3, 5};
